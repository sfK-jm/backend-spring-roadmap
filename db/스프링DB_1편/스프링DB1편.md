...
- `NetworkClient`는 외부 네트워크에 접속해서 어떤 기능을 처맇는 객체이다. 여기서는 `ConnectException`체크 예외를 던진다.
- 서비스는 리포지토리와 `NetworkClient`를 둘다 호출한다.
  - 따라서 두 곳에서 올라오는 체크 예외인 `SELExceptoin`과 `ConnectException`을 처리해야 한다.
...
- JDBC -> JPA 같은 기술로 변경하면 예외도 함께 변경해야 한다. 그리고 해당 예외를 던지는 모든 다음 부분도 변경해야 한다.
  - `logic() throws SQLException` -> `logic() throws JPAException`(참고로 JPA 예외는 시렞 이렇지는 않고, 이해하기 쉽게 예를 든 것이다.)

### 정리

- 처리할 수 있는 체크 예외라면 서비스나 컨트롤러에서 처리하겠지만, 지금처러 데이터베이스나 네트워크 통신처럼 시스템 레벨에서 올라온 예외들은 대부분 복구가 불가능하다. 그리고 실무에서 발생하는 대부분의 예외들은 이런 시스템 예외들이다.
...
- **throws Exception**
  ...
  - 그런데 다음과 같이 최사우이 예외인 `Exception`을 던져도 문제를 해결할 수 있다. (그러면 `SQLException`, `ConnectException` 에 의존하지 않아도 되므로, 의존관계 문제는 해결된다.)
